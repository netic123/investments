...
test('the Fear & Greed header and notes describe the last completed bar, one decimal, and the studied score versions', () => {
  ...
  assert.match(html, /composite of the benchmark’s last bar dated before the build day; \$\{carried\.length\} of \$\{total\} indicators carried from \$\{carriedDates\.length>1\?carriedDates\.map\(fmtDate\)\.join\(" \/ "\):fmtDate\(oldest\)\}/);
