...
This historical research runner tests whether the repository's frozen version-1/version-2 Fear & Greed definitions, applied to Crypto, Sweden, USA, Europe and Global, predicted later market returns. Its five frozen signals use trailing 252-observation percentile scoring, a 126-valid-observation minimum, all six components, fixed directions, equal weights and labels. Production version 3 instead uses causal expanding percentiles over all supplied history and is a different, unvalidated model; version-1/version-2 artifacts must not be presented as evidence for it. The configured raw proxies differ; volatility is an implied-volatility level where configured and benchmark realized volatility otherwise. The runner is dependency-free and uses Node's built-in `fetch`, crypto, and filesystem APIs. It does not download or use any published third-party Fear & Greed score.
